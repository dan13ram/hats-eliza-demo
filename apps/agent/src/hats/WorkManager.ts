...
export interface GoalManager {
    ...
    saveGoal(_goal: string): Promise<void>;
    getBudget(): Promise<bigint | null>;
    saveBudget(_budget: bigint): Promise<void>;
}

export class HatsGoalManager implements GoalManager {
    /*
        storeId: string = "";
    ...
    private formatCurrency(amount: number | bigint): string {
        return `${formatEther(BigInt(amount))} ETH`;
    }

    // Cache Methods
    async getGoal(): Promise<string | null> {
        const cachedGoal = await this.runtime.cacheManager.get<string>(`goal`);
        return cachedGoal || null;
    }

    async saveGoal(goal: string): Promise<void> {
        await this.runtime.cacheManager.set(`goal`, goal);
    }

    async getBudget(): Promise<bigint | null> {
        const cachedBudget = await this.runtime.cacheManager.get<bigint>(`budget`);
        return cachedBudget ? BigInt(cachedBudget) : null;
    }

    async saveBudget(budget: bigint): Promise<void> {
        await this.runtime.cacheManager.set(`budget`, budget);
    }
    ...
    /*
        ...
        getOrderSummary(order: Order, customer: Customer): string {
            ...
            return `Order Summary:\n${order.items.map(item =>
                `- ${item.quantity}x ${item.size} ${item.crust} Pizza${item.toppings?.length ? ` with ${item.toppings.map(t =>
